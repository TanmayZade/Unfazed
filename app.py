...
def register():
    ...
    sheet.append([timestamp, first_name, last_name, email, phone, username, generate_password_hash(password)])
    workbook.save(REGISTRATION_FILE)
...
def report():
    ...
    user_email = 'tanmayzade87@gmail.com'  # Hardcoded user email
...
def login():
    ...
    for row in sheet.iter_rows(min_row=2, values_only=True):
        if row[5] == username and check_password_hash(row[6], password):  # Username at index 5 and Password at index 6
...
def update_status():
    ...
    user_email = 'tanmayzade87@gmail.com'  # Hardcoded user email
    user_name = 'Tanmay'  # You may want to retrieve this from the Excel as well
